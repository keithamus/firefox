...
void Document::CloseEntirePopoverList(PopoverAttributeState aMode,
  ...
  for (auto popoverList = PopoverListOf(aMode); !popoverList.IsEmpty();
       popoverList = PopoverListOf(aMode)) {
    RefPtr topmost = popoverList.LastElement();
...
void Document::HidePopoverStackUntil(PopoverAttributeState aMode,
  ...
  MOZ_ASSERT(nsGenericHTMLElement::FromNode(&aEndpoint)
                 ->IsPopoverOpenedInMode(aMode));
